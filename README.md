...
#### Диаграмма состояний
![Диаграмма состояний](./lr2.png)

#### Пример работы
![Пример работы](./lr2(test).png)
...
#### Пример работы
![Результат поиска по регулярному выражению](./lr6.png)
...
### Вариант: №7  
Реализация алгоритма синтаксического анализа на основе метода **рекурсивного спуска** по заданной грамматике.
...
```ebnf
1. <expr>           ::= <expr_add_sub> | <expr> <operator_mul_div> <expr_add_sub>
2. <expr_add_sub>   ::= <term> | <expr_add_sub> <operator_add_sub> <term>
3. <term>           ::= <integer> | "(" <expr> ")"
4. <operator_mul_div> ::= "*" | "/" | "%"
5. <operator_add_sub> ::= "+" | "-"
...
- **Тип**: Контекстно-свободная грамматика (КС-грамматика)
- **Вид**: Левая рекурсивная, допускающая преобразование в LL(1)
- **Назначение**: Описание структуры арифметических выражений над целыми числами с операциями `+ - * / %` и скобками
...
- **Язык**: Python 3
- **Библиотеки**: `PyQt6`, собственные модули: `scanner.py`, `recursive_parser.py`, `interf.py`

---

## Схема вызова функций рекурсивного синтаксического анализатора

```text
parse_expr
 └── parse_expr_add_sub
      ├── parse_term
      │    └── parse_integer | parse_expr (рекурсивно)
      ├── match("+"/"-")
      └── parse_term
```
...
- Успешный разбор, построено дерево вызовов.
- Все лексемы корректны.
- Лог: `parse_expr → parse_expr_add_sub → parse_term → ...`
![image](https://github.com/user-attachments/assets/32a4a6f3-cb95-4289-bf29-5f5c02b4c9f3)
...
(12 + 5 - 7) *  /
```
![image](https://github.com/user-attachments/assets/ac222288-8b8e-4f0e-bbe3-559430b4d31e)
...
```text
graph TD
    Start --> Digit[Цифра] --> Integer[Целое число]
    Start --> Op[+ - * / %] --> Operator
    Start --> Paren["(" или ")"] --> ParenOK
    Start --> Letter[Буква] --> Identifier[Идентификатор]
    Start --> Unknown[Иной символ] --> Error[Ошибка]
```
Диаграмма описывает переходы между состояниями сканера для различных типов символов.

### Пример с лексическими ошибками
```
12 + abc$ - 7.5)
```
![image](https://github.com/user-attachments/assets/bbaf7a95-814a-4594-96fa-439521810cd6)

```
(12 + @5 - 7) *  /
```
![image](https://github.com/user-attachments/assets/43ae2e8b-81e7-4ae4-bfaf-7d4f15e1ab22)

- Обнаружены лексические ошибки:
  - `@` — недопустимый символ
  - '$' — недопустимый символ
